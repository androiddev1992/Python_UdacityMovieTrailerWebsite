...
'''
# A actor details html template
actor_div_content = '''
<span class="person">
    <img src="{image}" width="32" height"32" alt="{name}"> {name}
</span>
'''

def create_movies_content(movies):
...
def create_person_span(actor):
    return actor_div_content.format(
        name=actor.movie_actor,
        image=actor.movie_actor_image if actor.movie_actor_image is not None else "http://static6.flixster.com/static/locker/images/actor.default.tmb.gif"
    )
...
def open_movies_page(movies):
    ...
    rendered_content = main_page_content.format(
        movie_tiles=create_movies_content(movies))
